...
class defaultType implements blogType {
	...
	public function getCoursePath($blti, $siteUrlArray, $domain) {
	    ...
	    $course = str_replace(':','-', $blti->getCourseKey());  // TO make it past sanitize_user
	...
	public function getMetaBlog($blti){
		...
		if (isset($blti->info['launch_presentation_locale']))
			$langid = $blti->info['launch_presentation_locale'];
		switch ($langid)
		{
			case "ca-ES":
			case "ca_utf8":
	...
	public function roleMapping($role, $blti) {
		...
		if (strpos($role , 'Administrator')!==FALSE) return "administrator";
	    if (strpos($role, 'Instructor')!==FALSE) return "editor";
